    ...
    async def _extract(self, state: EtlInscripcionesState) -> dict[str, Any]:
        try:
            items: list[EtlBaseState] = await self._extractor.extract_pipeline(document_data=self.document_data,
                                                                               origin="inscripciones")
            children: list[EtlInscripcionChild] = WorkflowService.resolve_inscripciones_children(items, state)
            if len(items) == 0:
                return {
    ...
    async def _load(self, state: EtlInscripcionesState) -> dict[str, Any]:
        try:
            ...
            if not transform_success:
                return {}
            children_transformed: list[EtlInscripcionChild] = state.children_transformed
            await anyio.to_thread.run_sync(self._loader.save_metadata, "inscripciones", children_transformed)
            return {
                "load_success": True
            }
        except Exception as e:
    ...
    async def _transform_unit(self, child: EtlInscripcionChild) -> EtlInscripcionChild | None:
        try:
            ...
            item: EtlInscripcionChild | None = await anyio.to_thread.run_sync(
                self._transformer.llm_caller_inscripciones,
                document_llm
            )
            if item is None:
            ...
            return EtlInscripcionChild(
                **child.model_dump(
                    exclude={"inscription_number", "legal_name", "inscription_date", "transform_success"}),
            ...
            self.logger.error(f"Error en el proceso de de carga de una unida en bedrock: {str(e)}")
            return None
